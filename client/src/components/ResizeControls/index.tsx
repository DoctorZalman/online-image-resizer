...
export function ResizeControls(): React.ReactElement | null {
  ...
  const handleResize = async (): Promise<void> => {
    ...
    try {
      await Promise.all(
        queuedJobs.map((job) => requestResize({ jobId: job.jobId, scalePercent: scale })),
      );
  ...
  return (
    <div className="flex flex-col gap-4 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">{scale}%</span>
      </div>
      ...
      >
        {isProcessing
          ? 'Processing...'
          : `Resize ${queuedJobs.length} file${queuedJobs.length !== 1 ? 's' : ''}`}
      </button>
    </div>
  );
}
